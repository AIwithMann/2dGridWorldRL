...
import random as r
class Env:
    def __init__(self, rows:int, cols:int, termitationStates:int)->None:
        self.rows = rows
        self.cols = cols
        self.termitationStates = termitationStates
        
        random_indices = np.random.choice(rows*cols, self.termitationStates, replace=False) 
        self.Rgrid = np.zeros((rows, cols)) #reward grid
        self.Rgrid.flat[random_indices] = 1 
        self.terminationIndices = np.where(self.Rgrid==1) #termination states
