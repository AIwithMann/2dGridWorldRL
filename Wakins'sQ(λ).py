    ...
    def train(self,alpha:float,e_min:int = 0.01, e_decay:float = 0.995)->None:
        for episode in range(self.maxIterations):
            ...
            while True:
                ...
                a_next = self.act()
                a_high = self.highestValueAction()
                delta = self.env.Rgrid[s_next[0],s_next[1]] + self. g * self.Qgrid[a_high,s_next[0],s_next[1]] - self.Qgrid[a,s[0],s[1]]
...
agent = Qlambda(env, 0.9,0.5,5000,0.5)
